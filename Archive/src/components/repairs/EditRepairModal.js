...
export default function EditRepairModal({ isOpen, onClose, repair, onSuccess, selectedCompletedTemplate }) {
  ...
  const { data: templates } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return templates.filter(t => t.templateType === "Completed Repair Email Template")
  ...
  useEffect(() => {
    if (repair && isMounted) {
      ...
      setSelectedTemplate(repair.fields['Completed Repair Template'] || '');
    }
  }, [repair, isMounted]);
  ...
  // Add this useEffect to sync with parent's selectedCompletedTemplate
  useEffect(() => {
    setSelectedTemplate(selectedCompletedTemplate || '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updateData = {
        ...
        'Completed Repair Template': selectedTemplate
      };
